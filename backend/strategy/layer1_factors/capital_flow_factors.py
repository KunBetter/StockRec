...
class MajorInflowFactor(BaseFactor):
    ...
    def compute(self, df: pd.DataFrame, main_net_inflow: pd.Series = None) -> pd.Series:
...
class NorthboundPctFactor(BaseFactor):
    ...
    def compute(self, df: pd.DataFrame, northbound_pct: pd.Series = None) -> pd.Series:
...
class MarginChangeFactor(BaseFactor):
    ...
    def compute(self, df: pd.DataFrame, margin_balance: pd.Series = None) -> pd.Series:
